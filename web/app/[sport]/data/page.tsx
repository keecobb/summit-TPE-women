...
export default async function DataPage({
  ...
  const [
    ...
    apiFetch<PlayerLeaderboard>(`/${sport}/leaderboards/players`, {
      params: {
    ...
    apiFetch<StandoutsLeaderboard>(`/${sport}/leaderboards/standouts`, {
      params: { level: "Low-Major", target_level: "High-Major", limit: 8 },
    }),
    apiFetch<StandoutsLeaderboard>(`/${sport}/leaderboards/standouts`, {
      params: { level: "Mid-Major", target_level: "High-Major", limit: 8 },
    }),
    apiFetch<OpponentSplitLeaderboard>(`/${sport}/leaderboards/opponent-splits`, {
    ...
    apiFetch<BackHalfLeaderboardAll>(`/${sport}/leaderboards/back-half`, {
    ...
    apiFetch<SeasonJumpLeaderboard>(`/${sport}/leaderboards/season-jump`, {
    ...
    apiFetch<TeamEfficiencyQuadrant>(`/${sport}/leaderboards/team-efficiency`, {
    ...
    apiFetch<PlayerEfficiencyQuadrant>(`/${sport}/leaderboards/player-efficiency`, {
    ...
    apiFetch<BestGamesLeaderboard>(`/${sport}/leaderboards/best-games`, {
  ...
  return (
    <div>
      ...
      <div className="card">
        <h2>Leaderboard</h2>
        <FilterForm className="filter-form" action={`/${sport}/data`}>
          <div className="field">
        ...
        <div className="pagination" style={{ marginTop: 12 }}>
          {leaderboardPage > 0 && (
            <Link className="btn" href={`/${sport}/data?${withLeaderboardPage(sp, leaderboardPage - 1)}`}>
              &larr; Previous
            </Link>
          )}
      ...
      <div className="card">
        ...
        <FilterForm className="filter-form" action={`/${sport}/data`} style={{ marginBottom: 0 }}>
          <div className="field">
      ...
      <div className="card">
        ...
        <BestGamesTable data={bestGames} sport={sport} />
      </div>
      ...
      <div className="card">
        ...
        <SeasonJumpTable data={seasonJump} sport={sport} />
      </div>
      ...
      <div className="card">
        ...
        <div className="card-grid card-grid-2">
          <div>
            ...
            <StandoutTable data={lmStandouts} sport={sport} />
          </div>
          <div>
            <h2 style={{ fontSize: "1.05rem" }}>Mid-Major &rarr; High-Major</h2>
            <StandoutTable data={mmStandouts} sport={sport} />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-grid card-grid-3">
          <div>
            ...
            <OpponentSplitTable data={hmVsHm} sport={sport} />
          </div>
          <div>
            <h2 style={{ fontSize: "1.05rem" }}>Mid-Major vs. HM</h2>
            <OpponentSplitTable data={mmVsHm} sport={sport} />
          </div>
          <div>
            <h2 style={{ fontSize: "1.05rem" }}>Low-Major vs. HM</h2>
            <OpponentSplitTable data={lmVsHm} sport={sport} />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-grid card-grid-3">
          <div>
            ...
            <OpponentSplitTable data={top50Hm} sport={sport} />
          </div>
          <div>
            <h2 style={{ fontSize: "1.05rem" }}>Mid-Major players</h2>
            <OpponentSplitTable data={top50Mm} sport={sport} />
          </div>
          <div>
            <h2 style={{ fontSize: "1.05rem" }}>Low-Major players</h2>
            <OpponentSplitTable data={top50Lm} sport={sport} />
          </div>
        </div>
      </div>
...
function StandoutTable({ data, sport }: { data: StandoutsLeaderboard; sport: string }) {
...
function OpponentSplitTable({ data, sport }: { data: OpponentSplitLeaderboard; sport: string }) {
  if (data.players.length === 0) return <p className="empty-state">No qualifying players.</p>;
  return (
    <div className="table-scroll">
      <table>
...
function BestGamesTable({ data, sport }: { data: BestGamesLeaderboard; sport: string }) {
        ...
        <tbody>
          {data.games.map((g, i) => (
            <tr key={`${g.player_id}-${g.date}-${i}`}>
              <td>
                <Link href={`/${sport}/players/${g.player_id}`}>{g.name}</Link>
                <div className="label" style={{ marginTop: 2 }}>
                  {g.team_name}
                </div>
              </td>
...
function SeasonJumpTable({ data, sport }: { data: SeasonJumpLeaderboard; sport: string }) {
  if (data.players.length === 0) return <p className="empty-state">No qualifying players.</p>;
  return (
    <div className="table-scroll">
      <table>
...
function BackHalfChangeChart({
  title, sub, players, getChange, getFirst, getSecond, unit, sport,
}: {
  title: string;
  sub: string;
      ...
      {players.map((p) => {
        ...
        return (
          <div className="bar-row" key={p.player_id}>
            <Link href={`/${sport}/players/${p.player_id}`} className="bar-name" title={`${p.name} (${p.team_name})`}>
              {p.name}
            </Link>
