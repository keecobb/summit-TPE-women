...
export default async function ConferenceStandingsPage({
  ...
  params: Promise<{ sport: string; conference: string }>;
}) {
  const { sport, conference } = await params;
  const name = decodeURIComponent(conference);

  let standings: ConferenceStandings;
  try {
    standings = await apiFetch<ConferenceStandings>(`/${sport}/conferences/${encodeURIComponent(name)}/standings`, {
      revalidate: 60,
    });
  } catch (e) {
  ...
  return (
    <div>
      ...
      <p style={{ marginTop: 20 }}>
        <Link href={`/${sport}/conferences`}>&larr; All conferences</Link>
      </p>
    </div>
  );
}
