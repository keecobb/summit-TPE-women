...
def team_needs(conn, team_id, top_n=3):
    """A team's roster-weighted per-40 category profile (team_profile,
    built once by build_cache.py) compared against the rest of the
    league. Returns every category ranked worst-to-best by z-score, with
    the worst top_n called out as `weaknesses`. Turnovers are flipped so
    a HIGH team turnover rate always reads as a negative z (a weakness),
    consistent with every other category where negative = below league
    average = weak.
    """
    ...
    for stat, info in CATEGORY_INFO.items():
        ...
        if val is None or mean_key not in meta:
            continue
...
def find_fits(conn, team_id, stat=None, limit=15, min_games=5):
    """Ranks every player NOT currently on team_id by her PROJECTED
    per-game contribution to `stat` if she transferred to team_id --
    reusing the exact same _core_projection() math as project_player(), so
    this ranking and a one-off /project call for the same player+team
    always agree. If `stat` isn't given, auto-picks team_id's single
    biggest weakness from team_needs().
 
    Minutes default to each candidate's own auto-projected fallback (no
    role/override) -- a fit ranking is meant to survey a wide pool, not
    assume every candidate walks into a specific role.
    """
    ...
    if stat is None:
        ...
        stat = needs["weaknesses"][0]["stat"]
    if stat not in CATEGORY_INFO:
        raise ProjectionError(f"stat must be one of {list(CATEGORY_INFO)}, got {stat!r}.")
    info = CATEGORY_INFO[stat]
    proj_field = info["proj_field"]
    ...
    cols = [d[0] for d in conn.execute("SELECT * FROM players LIMIT 0").description]
    candidates = [
        dict(zip(cols, row)) for row in
        conn.execute("SELECT * FROM players WHERE team_id != ? AND games >= ?", (team_id, min_games)).fetchall()
    ]
    ...
    for player in candidates:
        ...
        try:
            ...
            continue
        value = result["projected"].get(proj_field)
        if value is None:
            continue
        ranked.append(dict(
            player_id=player["player_id"], name=player["name"], position=player["position"],
            class_year=player["class_year"], current_team=current_team["name"],
            current_tier=current_team["tier"],
            projected=dict(minutes=result["projected"]["minutes"], **{proj_field: value}),
    ...
    ranked.sort(key=lambda r: r["fit_value"], reverse=not info["lower_is_better"])
    for r in ranked:
        del r["fit_value"]
 
    return dict(
        team_id=team_id, team_name=target_team["name"], stat=stat, stat_label=info["label"],
