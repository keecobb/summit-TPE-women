"""Main D1 scraper (men's): rosters, schedules, and full box scores for
every D1 team across the target seasons, written into MensSummitTPE.xlsx.

Mirrors scrapers/run_d1_scrape.py (the women's version) exactly in
structure and logic -- same resumability model, same PlayerSeasons
rebuild-from-PlayerGameStats design, same transfer-history rollup --
just pointed at scrapers.espn_client_mbb and MensSummitTPE.xlsx instead.
See that file's own comments for the full reasoning behind each design
decision below; this file intentionally keeps them rather than
shortening, since the same bugs they guard against (the PlayerSeasons
wipe bug, the mid-write truncation bug, etc.) apply equally here.

Run scrapers/team_match_mbb.py first so Teams.'ESPN Team ID' is filled
in for D1 rows -- teams without one are skipped here.
...
Player identity across seasons/schools: every player is keyed by his
...
second Player ID just because he played for two different teams
across the 3 seasons. Players.'Team ID' always reflects his CURRENT
team (see finalize_players() below) -- his season-by-season history,
...
don't include height/class at all. So:
  - Height doesn't change season to season, so once we know it (from
    whichever team's current roster lists his) we backfill it onto
    every one of his PlayerSeasons rows, including past seasons.
  ...
  - A player who already graduated or left Division I before the
    current season won't appear on anyone's current roster, so his
    Height/Class stay blank here. (ESPN also has a separate per-athlete
    endpoint -- espn_client_mbb.get_athlete_height() -- that can fill in
...
Usage:
    python scrapers/run_d1_mbb_scrape.py
    python scrapers/run_d1_mbb_scrape.py --seasons 2024 2025 2026
    python scrapers/run_d1_mbb_scrape.py --only "Duke,Houston"
"""
...
class Context:
    ...
    def note_roster_entry(self, espn_athlete_id, height, class_year, team_id, division, roster_season):
        ...
        this naturally reflects wherever he is *now* -- last call wins,
    ...
    for espn_team_id, rows in box.items():
        ...
        for player_row in rows:
            ...
            if player_row.get("did_not_play"):
                ...
                # transfer's post-move box scores (Dallan Coleman,
                # Georgia Tech -> UCF). Writing a PlayerGameStats row for
                # these inflates Games Played with games the player never
                # actually appeared in (found via 4 players whose Games
                # Played exceeded any real season's game count -- root
                # cause traced to this). Skipping them here is a one-line
                # fix; get_boxscore() already parses/returns this flag,
                # it just wasn't being checked by this loop before.
                continue
...
def process_team(ctx, info, seasons, live_season=None):
    """live_season: an in-progress season (e.g. this year's, while games are
    still being played) that should NEVER be written into done_seasons, no
    matter how cleanly it processes this run -- so every future run keeps
    re-checking its schedule for newly-completed games instead of treating
    it as permanently finished the moment one run happens to complete
    without an ESPN error. Confirmed this was a real gap (not hypothetical)
    with a live test before adding this: a season that was only PARTIALLY
    played so far still got added to done_seasons on a clean run, and a
    second run a "week later" (more games completed in between) then
    skipped the team entirely via the done_seasons/status=="Done" shortcut
    in main() -- the newly-completed games were silently never picked up.
    Re-checking an in-progress season's schedule every run is cheap even
    for a team with most of its games already recorded: process_game()
    skips straight past any Game ID already in ctx.games_seen before ever
    calling get_boxscore(), so only the genuinely-new completed game(s)
    since the last run actually cost an extra HTTP call.
    """
    ...
    for season in seasons:
        is_live = live_season is not None and str(season) == str(live_season)
        if str(season) in info["done_seasons"] and not is_live:
            continue
        ...
        if season_error:
            ...
            # For the live season, deliberately DON'T add it to
            # done_seasons even on a clean run -- see the live_season
            # docstring note above for why.
            ctx.mark_team_status(
                info, "In Progress", error=None,
                add_season=None if is_live else season,
            )
...
def finalize_players(ctx, current_season_label):
    ...
    2. For players not on any current roster (graduated / left the
       ...
       espn_client_mbb.get_athlete_height(), that CAN return Height for any
    ...
    4. Roll each player's season history into a single human-readable
       ...
       this run would silently vanish from his timeline -- this was a
       real, confirmed bug on the women's version of this script (see
       scrapers/run_d1_scrape.py's own comment on this point) before
       the PlayerGameStats-sourced rebuild replaced the old in-memory
       accumulator; that fix is carried over here from the start rather
       than needing to be rediscovered against men's data.
    """
    ...
    # One line per player: his schools in chronological order, so a
...
def main():
    ...
    parser.add_argument(
        "--live-season", type=int, default=None,
        help=(
            "The currently-in-progress season (e.g. 2027 for 2026-27), if "
            "any is in --seasons. That season is NEVER written into "
            "done_seasons, so every run -- run it weekly during the season "
            "-- keeps re-checking its schedule for newly-completed games "
            "instead of treating it as permanently finished after the "
            "first clean run. Already-recorded games are still skipped "
            "cheaply via the Game ID dedup, so a weekly re-run only costs "
            "an extra HTTP call for the genuinely-new games since last "
            "time, not a full re-scrape. Omit this for a normal backfill "
            "run over seasons that have already fully concluded (the "
            "original, unchanged behavior)."
        ),
    )
    args = parser.parse_args()

    print(f"run_d1_mbb_scrape.py -- last revised {LAST_REVISED}")
    ...
    live_note = f", live season {args.live_season} (always rechecked)" if args.live_season else ""
    print(f"{len(targets)} D1 teams to process, seasons {args.seasons}{live_note}.")

    for i, info in enumerate(targets, start=1):
        already_done = set(str(s) for s in args.seasons) <= info["done_seasons"] and info["status"] == "Done"
        # If a live season was requested, never take the whole-team
        # shortcut -- process_team() still skips each already-finished
        # PAST season instantly (no API call), but the live season's
        # schedule always gets rechecked for this team. See
        # process_team()'s live_season docstring for the full reasoning.
        if already_done and not args.live_season:
            continue
        print(f"[{i}/{len(targets)}] {info['name']} (ESPN ID {info['espn_id']}) ...")
        try:
            process_team(ctx, info, args.seasons, live_season=args.live_season)
