"""Refreshes summit_tpe_cache.sqlite from a workbook that's already been
updated (scraped/backfilled) -- the mechanical, safe-to-automate half of
keeping a live site's data current, without the manual "SSH in, run
build_cache.py, restart uvicorn" dance, and without a live site ever
serving a half-written or known-bad cache.
...
that point:
  ...
  2. build_cache.py, writing to a TEMP file, not the live path -- so a
     build that fails partway through (crash, disk full, whatever) never
     leaves the live cache half-written.
  3. Backs up the current live cache to summit_tpe_cache.sqlite.bak (one
     generation of rollback) if one exists.
  4. Atomically swaps the temp file into the live path (os.replace).
 
No API restart is needed after this: api.py's get_conn() opens a fresh
sqlite3 connection per request rather than holding one open for the
...
Usage:
    python refresh_pipeline.py --path WomensSummitTPE.xlsx
    python refresh_pipeline.py --path WomensSummitTPE.xlsx --out summit_tpe_cache.sqlite
    python refresh_pipeline.py --path WomensSummitTPE.xlsx --skip-health-check   # not recommended
    python refresh_pipeline.py --path WomensSummitTPE.xlsx --max-roster 30       # passed through to check_data_health.py
...
def main():
    ...
    parser.add_argument("--path", required=True, help="Path to the already-updated WomensSummitTPE.xlsx")
    parser.add_argument("--out", default="summit_tpe_cache.sqlite", help="Live cache path api.py reads from")
    ...
    live_path = HERE / args.out
    tmp_path = HERE / f"{args.out}.building-{int(time.time())}"
    bak_path = HERE / f"{args.out}.bak"
    ...
    ok = run_step("2/3 Building cache to a temp file",
                   [sys.executable, "build_cache.py", "--path", str(workbook), "--out", str(tmp_path)])
    if not ok:
